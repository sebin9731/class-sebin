/*
    ...
    1. DDL
        ...
        - 데이터 정의어
        - DB의 구조를 생성/관리하는 명령어
        - 데이터베이스 객체(테이블, 뷰, 사용자, 인덱스 등)를 생성/수정/삭제하는 명령어
        a. create: 생성
        b. drop: 삭제
        c. alter: 수정
    ...
    2. PRIMARY KEY, PK
        ...
        - 반드시 테이블에는 PK가 존재해야 한다.(PK 없는 테이블 존재 가능 > 안 만듦)
    
    3. FOREIGN KEY
    
    4. UNIQUE
        - 유일하다. 레코드 간에 중복값을 가질 수 없다.
    ...
    [6. DEFAULT]
        - 기본값 설정 기능
        - insert/update 작업 시 > 컬럼에 값을 안 넣으면 null 대신에 미리 설정한 값을 대신 대입
...
create table tblMemo (
    --컬럼명 자료형(길이) NULL 제약사항
    seq number(3) null, -- 이 컬럼은 값을 안 넣어도 됩니다(optional) 생략 가능
...
--ORA-00001: 무결성 제약 조건(HR.SYS_C008623)에 위배됩니다
insert into tblMemo (seq, name, memo, regdate)
...
--ORA-00001: 무결성 제약 조건(HR.SYS_C008693)에 위배됩니다
insert into tblMemo (seq, name, memo, regdate)
    values (3, '강아지', '왈왈왈', sysdate);
...
insert into tblMemo (seq, name, memo, regdate, priority, category) values (1, '강아지', '멍멍멍', sysdate, 1, '할일');
insert into tblMemo (seq, name, memo, regdate, priority, category) values (2, '고양이', '야옹', sysdate, 2, '공부');
--ORA-02290: 체크 제약조건(HR.TBLMEMO_PRIORITY_CK)이 위배되었습니다
insert into tblMemo (seq, name, memo, regdate, priority, category) values (3, '병아리', '삐약.', sysdate, 5, '할일');
--ORA-02290: 체크 제약조건(HR.TBLMEMO_CATEGORY_CK)이 위배되었습니다
insert into tblMemo (seq, name, memo, regdate, priority, category) values (3, '병아리', '삐약.', sysdate, 2, '여행');
--ORA-02290: 체크 제약조건(HR.TBLMEMO_CATEGORY_CK)이 위배되었습니다
insert into tblMemo (seq, name, memo, regdate, priority, category) values (3, '송아지', '음매.', sysdate, 2, '시험');
...
insert into tblMemo (seq, name, memo, regdate)
    values (2, null, '멍멍멍', sysdate); -- null > default 동작 안 함

-- 암시적 null 대입 > 사용자 의지가 없다. + 권리 포기
insert into tblMemo (seq, memo, regdate)
    values (3, '멍멍멍', sysdate); --null 대신 '익명' > default 동작함
...
create table tblMemo (
    ...
    constraint tblmemo_memp_ck check (length(memo) >= 10)
);
