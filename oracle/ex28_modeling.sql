/*
    ...
    데이터 모델링
    - 요구 분석 기반 > 아이디, 암호 > 수집된 데이터 > 분석 > 어떻게 저장? > 저장할 구조 > 도식화 > 산출물(ERD)
    ...
    4. Relationship
    - 엔티티와 엔티티 간의 관계
    - 하나의 엔티티 내의 속성과 또 다른 엔티티 내의 속성 간의 관계
    - 테이블.컬럼(PK) <-> 테이블.컬럼(FK)
    ...
    정규화, Normalization
    - 자료의 손실이나, 불필요한 정보를 없애고, 데이터의 일관성을 유지하며, 데이터의 종속성을 최소화하기 위해 ERD를 수정하는 작업
    ...
    - 정규화는 '부분 함수 종속'이나 '이행 함수 종속'을 제거하고, 모든 컬럼의 관계를 '완전 함수 종속'으로 만드는 작업이다.
    1. 부분 함수 종속 X
    2. 이행 함수 종속 X
    3. 완전 함수 종속 O
